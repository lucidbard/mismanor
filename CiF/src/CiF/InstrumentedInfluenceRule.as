package CiF 
{
	...
	public class InstrumentedInfluenceRule extends InfluenceRule 
	{
		...
		public override function evaluate(initiator:GameObject, responder:GameObject, other:GameObject= null, sg:SocialGame=null):Boolean {
			...
			this.addTruthStateToHistory(initiator as Character, responder  as Character, other  as Character, truthState);

			return isTrue;
		}
		
		public override function evaluateTimeOrdedRule(x:GameObject, y:GameObject, z:GameObject):Boolean {
			...
			truthState["result"] = isTrue;
			this.addTruthStateToHistory(x  as Character, y  as Character, z as Character, truthState);
		...
		private function addTruthStateToHistory(x:Character, y:Character, z:Character, truthState:Dictionary):void {
			...
			var zName:String = (z)?z.name:"none";
			...
			if (!(x.name in this.history[time])) {
				(this.history[time] as Dictionary)[x.name] = new Dictionary();
			}
			
			//is there a dictionary for this y?
			if (!this.history[time][x.name][y.name]) {
				this.history[time][x.name][y.name] = new Dictionary();
			}

			//is there a dictionary for this z?
			if (!this.history[time][x.name][y.name][zName]) {
				this.history[time][x.name][y.name][zName] = new Dictionary();
			}

			//Add the truthState.
			this.history[time][x.name][y.name][zName] = truthState;
		}
