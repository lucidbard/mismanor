package CiF 
{
	...
	public class Rule
	{
		...
		public function sortPredicates():void
		{
			...
			/*var knowledgePreds:Vector.<Predicate> = new Vector.<Predicate>();
			var giveKnowledgePreds:Vector.<Predicate> = new Vector.<Predicate>();*/
...
/*						case Predicate.KNOWLEDGE:
							knowledgePreds.push(pred);
							break;
						case Predicate.GIVEKNOWLEDGE:
							giveKnowledgePreds.push(pred);
							break;*/
						default:
							trace("Predicate " + pred + " of type " + pred.type + " not found.");
					}
				}
			}
...
/*			knowledgePreds = knowledgePreds.sort(comparePredicateNames);
			giveKnowledgePreds = giveKnowledgePreds.sort(comparePredicateNames);*/
...
/*			for each (p in knowledgePreds) this.predicates.push(p);
			for each (p in giveKnowledgePreds) this.predicates.push(p);*/
		}
...
/*				case Predicate.GIVEKNOWLEDGE:
				case Predicate.KNOWLEDGE:
					if (x.knowledge < y.knowledge) {
						return -1;
					}
					else if (x.knowledge > y.knowledge) {
						return 1;
					}
					else return 0;
					break;*/
		 ...
		 * false.
		 */
		public function evaluate(initiator:GameObject, responder:GameObject, other:GameObject = null, sg:SocialGame=null):Boolean {
			this.lastTrueCount = 0;
			var p:Predicate;
			var p1:Predicate;
			...
			/*if (this.name)
			{
				if (this.name == "fooo")
				{
					...
					{
						Debug.debug(this, "there are two chars and the third is: " + ((other)?other.name:"NULLZ"));
						if (initiator.name.toLowerCase() == "monica" && responder.name.toLowerCase() == "zack")
						{
		 ...
		 */
		public function evaluateTimeOrdedRule(x:GameObject, y:GameObject, z:GameObject):Boolean {
		 ...
		 * false.
		 */
		public function evaluateAllNonIntent(initiator:GameObject, responder:GameObject, other:GameObject = null, sg:SocialGame=null ):Boolean {
			this.lastTrueCount = 0;
			for each (var p:Predicate in this.predicates) {
				//Debug.debug(this, "evaluating predicate of type: " + Predicate.getNameByType(p.type), 0);
				if(!p.intent) {
		 ...
		 * @param	other		A third party in the social game.
		 */
		public function valuation(initiator:GameObject, responder:GameObject, other:GameObject = null, sg:SocialGame=null):void {
		...
		public function toNaturalString(initName:String, respName:String, otherName:String):String {
			var returnstr:String = "";
			
			for each (var pred:Predicate in this.predicates)
				returnstr += pred.toNaturalLanguageString(initName, respName, otherName);
		...
		public function toXML():XML {
			var outXML:XML = <Rule name={this.name} id={this.id} description={this.description}></Rule>;
			for each (var pred:Predicate in this.predicates) {
		...
		public function generateRuleName():String {
			...
			for (var i:int = 0; i < this.predicates.length; i++) {
				...
				if (predicateName.toLowerCase() == "network") {
					...
					switch(tempNetworkString) {
						...
						case "TrustNetMed":
							...
							break;
						case "FamilyBondNetMed":
							if (!tempNetworkBooleanArray[3]) tempNetworkBooleanArray[3] = true;
							else continue;
							break;
					...
					//trace("CiF.Rule.as: " + this.predicates[i]);
				...
				else if (predicateName.toLowerCase() == "knowledge") {
					ruleName += this.predicates[i].toString();
				}
				else if (predicateName.toLowerCase() == "giveknowledge") {
					ruleName += this.predicates[i].toString();
				}*/
		...
		private function generateIntentString(intentIndex:int):String {
			...
			if (Predicate.getNameByType(intentType) == "network") {
				...
				else if (networkType == "trust") generatedRuleName += "Trust";
				else if (networkType == "familybond") generatedRuleName += "FamilyBond";
		...
		private function generateNetworkPredicateString(predicateIndex:int):String {
			...
			switch(SocialNetwork.getNameFromType(this.predicates[predicateIndex].networkType)) {
				...
				case "trust": tempNetworkType += "TrustNet"; break;
				case "familybond": tempNetworkType += "FamilyBondNet"; break;
			...
			else ruleName += tempNetworkType + "Non-Standard Network Value Used";
		...
		private function generateStatusPredicateString(predicateIndex:int):String {
			...
			var statusIndex:Number = this.predicates[predicateIndex].status;
			/*
			...
			//predicateString += "(";
			if (this.predicates[predicateIndex].primary == "initiator") {
				predicateString += "initiator";
			}
			else if (this.predicates[predicateIndex].primary == "responder") {
				predicateString += "responder";
			}
			else if (this.predicates[predicateIndex].primary == "other") {
				predicateString += "other";
			}
			//predicateString += ")";
			if (statusIndex < Status.FIRST_DIRECTED_STATUS){
				predicateString += " is ";
				predicateString += Status.getStatusNameByNumber(statusIndex);
			} else {
				predicateString += Status.getStatusNameByNumber(statusIndex);
				//predicateString += " toward ";
				predicateString += generateDirected(predicateIndex);
			}
		...
		private function generateTraitPredicateString(predicateIndex:int):String {
			//trace("CiF.GenerateTraitPredicateString: " + predicateIndex);
			var traitString:String = "";
			//traitString += "(";
			if (this.predicates[predicateIndex].primary == "initiator") {
				traitString += "initiator";
			}
			else if (this.predicates[predicateIndex].primary == "responder") {
				traitString += "responder";
			}
			else if (this.predicates[predicateIndex].primary == "other") {
				traitString += "other";
			}
			traitString += " is ";
			traitString += Trait.getNameByNumber(this.predicates[predicateIndex].trait);
			return traitString;
		}
