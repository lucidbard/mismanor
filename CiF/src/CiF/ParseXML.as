	...
	public class ParseXML
	{
		...
		public static function predicateParse(predXML:XML):Predicate {
			...
			if (type == "trait") {
				...
				if (predXML.@isSFDB == "true") 
					sfdbBool = true;	
				p.setStatusPredicate(predXML.@first, predXML.@second, Status.getStatusNumberByName(predXML.@status), negatedBool, Number(predXML.@duration), sfdbBool);
			}
			...
			if (predXML.@intent == "true")
			{
				p.intent = true;
				p.intentType = predXML.@intentType;
			}
		...
		public static function castParse(castXML:XML):void {
			...
			for each(characterXML in castXML.Character) {
				...
				character.name = characterXML.@name;
			...
			for each(characterXML in castXML.Character) {
				traitAndStatusParse(characterXML);
			}
		}
		
		/** (April)
		 * Parses the XML representation of the cast and populates the Item List
		 * singleton.
		 * @param	itemXML The XML representation of the cast.
		 */
		public static function itemParse(itemListXML:XML):void {
			var cif:CiFSingleton = CiFSingleton.getInstance();
			var item:Item;
			var itemXML:XML;
			...
			for each(itemXML in itemListXML.Item) {
				...
				item.networkID = itemXML.@networkID;
				item.traits.push(Trait.ITEM); // Make sure each item is marked as an item

				cif.itemList.addItem(item);
			}
			// Assign their traits and statuses
			for each(itemXML in itemListXML.Item) {
				traitAndStatusParse(itemXML);
			}
		}
		
		/** (April)
		 * Parses the XML representation of the cast and populates the Knowledge List
		 * singleton.
		 * @param	knowledgeXML The XML representation of the cast.
		 */
		public static function knowledgeParse(knowledgeListXML:XML):void {
			var cif:CiFSingleton = CiFSingleton.getInstance();
			var knowledge:Knowledge;
			var knowledgeXML:XML;
			
			//make the Character instantiations and put them in the Cast with names and network IDs
			for each(knowledgeXML in knowledgeListXML.Knowledge) {
				knowledge = new Knowledge();
				knowledge.name = knowledgeXML.@name;
				knowledge.networkID = knowledgeXML.@networkID;

				cif.knowledgeList.addKnowledge(knowledge);
			}
			// Assign their traits and statuses
			for each(knowledgeXML in knowledgeListXML.Knowledge) {
				traitAndStatusParse(knowledgeXML);
			}
		}
		 ...
		 * */
		public static function traitAndStatusParse(objXML:XML):void {
			var cif:CiFSingleton = CiFSingleton.getInstance();
			var currentObj:GameObject = cif.getGameObjByName(objXML.@name);
			...
			for each(var statusXML:XML in objXML..Status) {
				//Debug.debug(ParseXML, "characterParse() statusXML: " + statusXML.toXMLString());
				currentObj.addStatus(Status.getStatusNumberByName(statusXML.@type), cif.getGameObjByName(statusXML.@to));
			}

			for each(var curTrait:XML in objXML..Trait) {
				currentObj.traits.push(Trait.getNumberByName(curTrait.attribute("type")));
			}
		...
		public static function microtheoryParse(microtheoryXML:XML):void {
			...
			{
				trace("microtheoryParse() " + ii + " Pos Weight Total: " + cif.intentsTotalPos[ii]);
...
/*			if (CONFIG::InstrumentRules) {
		...
		public static function ruleParse(ruleXML:XML):Rule {
			...
			r.name = ruleXML.@name;
			r.description = ruleXML.@description;
		...
		public static function buddyNetworkParse(network:XML):void {
			...
			for each(var weight:XML in buddyWeights) {
				budNet.setWeight(cif.getGameObjByName(weight.@from).networkID, cif.getGameObjByName(weight.@to).networkID, weight.@value);
			}
		 ...
		 */
		public static function trustNetworkParse(network:XML):void {
			var trustNet:TrustNetwork = TrustNetwork.getInstance();
			var cif:CiFSingleton = CiFSingleton.getInstance();
			trustNet.initialize(network.@numChars);
			var trustWeights:XMLList = network.children();
			for each(var weight:XML in trustWeights) {
				trustNet.setWeight(cif.getGameObjByName(weight.@from).networkID, cif.getGameObjByName(weight.@to).networkID, weight.@value);
			}
		...
		public static function familyBondNetworkParse(network:XML):void {
			var familyNet:FamilyBondNetwork = FamilyBondNetwork.getInstance();
			var cif:CiFSingleton = CiFSingleton.getInstance();
			familyNet.initialize(network.@numChars);
			var familyWeights:XMLList = network.children();
			for each(var weight:XML in familyWeights) {
				familyNet.setWeight(cif.getGameObjByName(weight.@from).networkID, cif.getGameObjByName(weight.@to).networkID, weight.@value);
			}
			//trace("ParseXML::coolNetworkParse Internal Representation: " + coolNet.toXMLString());
			//trace("ParseXML::coolNetworkParse XML File: " + network);
		}
		...
		public static function romanceNetworkParse(network:XML):void {
			...
			for each(var weight:XML in romanceWeights) {
				...
				romanceNet.setWeight(cif.getGameObjByName(weight.@from).networkID, cif.getGameObjByName(weight.@to).networkID, weight.@value);
			}
		...
		*/
		public static function statusesParse(network:XML):void {
			...
			for each(var stat:XML in statusList) {
				...
				if(stat.@to == ""){
					char.addStatus(Status.getStatusNumberByName(stat.@type), null, stat.@duration);
				}
				else{
					char.addStatus(Status.getStatusNumberByName(stat.@type), theCast.getCharByName(stat.@to), stat.@duration);
				}
			}
		}
		...
		*/
		...
		public static function parseSocialGame(sgXML:XML):SocialGame
		{
			...
			// (April) this is what allows the user to select an effect for certain moves
			if (SocialGame.checkEffectSelection(sg.name)) sg.needsSecondOther = true;
			
			//trace("parsing XML: " + sgXML.@type);
			sg.responderType = Number(sgXML.@type);
			sg.otherType = Number(sgXML.@othertype);
			(sgXML.@requiresOther == "true")?sg.requiresOther = true : sg.requiresOther = false;
			...
			for each (effXML in sgXML.Effects..Effect) {
				...
				e.rejectID = Number(effXML.@rejectID);
		...
		public static function addGameToLibrary(myXML:XML):void
		{
			...
			for each (var sgXML:XML in myXML..SocialGame) {
				...
				{
					// Design tool doesn't allow this anymore
