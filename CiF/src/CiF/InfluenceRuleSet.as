package CiF 
{
	...
	public class InfluenceRuleSet
	{
		...
		public function clean(): void {
			...
			this.lastTruthValues = null;
			if (this.influenceRules)
			{
		 ...
		 * rules.
		 */
		public function scoreRulesWithVariableOther(initiator:GameObject, responder:GameObject, other:GameObject = null, sg:SocialGame = null, activeOtherCast:Vector.<GameObject> = null, microtheoryName:String = "", isResponder:Boolean = false):Number {
			...
			for each (var ir:InfluenceRule in this.influenceRules) 
			{
				if (ir.weight != 0)
				{
					...
					{
						for each (var other1:GameObject in possibleOthers)
						{
							if (other1.name != initiator.name && other1.name != responder.name)
							{
								if (ir.evaluate(initiator, responder, other1, sg))
								{
									...
									ruleRecord.other = other1.name;
									if (isResponder && responder.type == GameObject.TYPE_CHARACTER)
									{
										var charResp1:Character = CiFSingleton.getInstance().cast.getCharByName(responder.name) as Character;
										charResp1.prospectiveMemory.responseSGRuleRecords.push(ruleRecord);
									}
									else if (initiator.type == GameObject.TYPE_CHARACTER)
									{
										var charInit1:Character = CiFSingleton.getInstance().cast.getCharByName(initiator.name) as Character;
										charInit1.prospectiveMemory.ruleRecords.push(ruleRecord);
									}
									
									// (April) Debug
			/*						if (ir.weight != 0){
										if (other != null) trace("CiF.IRS: " + ir.generateRuleName() + " " + ir.weight + " ... " + initiator.name + " " + responder.name + " " + other.name);
										else trace("CiF.IRS: " + ir.generateRuleName() + " " + ir.weight + " ... " + initiator.name + " " + responder.name);
									}
						*/	
									score += ir.weight;
					...
					else
					{
						if (ir.evaluate(initiator, responder, null, sg))
						{
							...
							if (isResponder && responder.type == GameObject.TYPE_CHARACTER)
							{
								var charResp2:Character = CiFSingleton.getInstance().cast.getCharByName(responder.name) as Character;
								charResp2.prospectiveMemory.responseSGRuleRecords.push(ruleRecord);
							}
							else if (initiator.type == GameObject.TYPE_CHARACTER)
							{
								var charInit2:Character = CiFSingleton.getInstance().cast.getCharByName(initiator.name) as Character;
								charInit2.prospectiveMemory.ruleRecords.push(ruleRecord);
							}
...
*/							
		 ...
		 * rules.
		 */
		public function scoreRules(initiator:GameObject, responder:GameObject, other:GameObject = null, sg:SocialGame = null, activeOtherCast:Vector.<GameObject> = null, microtheoryName:String = "", isResponder:Boolean = false):Number 
		{
			var possibleOthers:Vector.<GameObject>;
			if (activeOtherCast != null) {
				possibleOthers = activeOtherCast;
	...
	*/						
					...
					else
					{
						...
						{							
							...
							if (other) ruleRecord.other = other.name;
							if (isResponder && responder.type == GameObject.TYPE_CHARACTER)
							{
								var charResp2:Character = CiFSingleton.getInstance().cast.getCharByName(responder.name) as Character;
								charResp2.prospectiveMemory.responseSGRuleRecords.push(ruleRecord);
