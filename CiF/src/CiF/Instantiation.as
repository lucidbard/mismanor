package CiF 
{
	...
	public class Instantiation
	{
		...
		public function clean(): void
		{
			...
			if (lines)
			{
				for each (var line:LineOfDialogue in lines)
				{
					line.clean();
					line = null;
				}
			...
			if (conditionalRules)
			{
				for each (var rule:Rule in this.conditionalRules)
				{
					rule.clean();
					rule = null;
				}
		...
		public function realizeDialogue(initiator:GameObject, responder:GameObject, other:GameObject = null):Instantiation 
		{
			...
			{
				Debug.debug(this, "line is " + line.initiatorLine + ", " + line.responderLine);
				newLod = line.clone();
				...
				if (CiFSingleton.getInstance().isInPromToolPreviewMode)
				{
					for each (locution in line.initiatorLocutions)
					{
						...
						toc1.renderText(initiator, responder, other);
						toc2.renderText(initiator, responder, other);
						toc3.renderText(initiator, responder, other);
				...
				else
				{
					for each (locution in line.initiatorLocutions)
					{
						...
						{
							newLod.initiatorLine += locution.renderText(initiator, null, null);
						}
						...
						else
						{
							...
							newLod.initiatorLine += locution.renderText(initiator, responder, other);
						}
					}
					...
					for each (locution in line.responderLocutions)
					{
						...
						{
							newLod.responderLine += locution.renderText(null, responder, null);
						}
						else if (locution.getType() == "ConditionalLocuation")
						{
							conditionalLocution = locution as ConditionalLocution;
						...
						else
						{
							...
							//Debug.debug(this, "realizeDialogue(): initiator locution: " + l);
							newLod.responderLine += locution.renderText(initiator, responder, other);
						}
					}
					//make the other line
					for each (locution in line.otherLocutions)
					{
						...
						{
							newLod.otherLine += locution.renderText(null, null, other);
						}
						else if (locution.getType() == "ConditionalLocuation")
						{
							conditionalLocution = locution as ConditionalLocution;
						...
						else
						{
							...
							//Debug.debug(this, "realizeDialogue(): initiator locution: " + l);
							newLod.otherLine += locution.renderText(initiator, responder, other);
						}
					}
