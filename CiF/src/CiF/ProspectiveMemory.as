...
,		 * 
		*/
		...
		public function clean():void {
			...
			if (this.ruleRecords)
			{
				for each (record in this.ruleRecords)
				{
					record.clean();
					record = null;
				}
				this.ruleRecords = null;
			}
			
			if (this.responseSGRuleRecords)
			{
				for each (record in this.responseSGRuleRecords)
				{
					record.clean();
					record = null;
				}
				this.responseSGRuleRecords = null;
			}
			intentScoreCache = null;
			intentPosScoreCache = null;
			intentNegScoreCache = null;
		...
		public function initializeIntentScoreCache():void 
		{
			...
			// (April) Adding other objects
			numChars += cif.itemList.items.length;
			numChars += cif.knowledgeList.knowledges.length;
		...
		public function getIntentScore(typeOfIntent:int, responder:GameObject):Number
		{
		 ...
		 * @return
		 */
		public function getGameScoreByGameName(gameName:String,responder:GameObject):GameScore
		{
			for each (var gs:GameScore in this.scores)
			{
				if (gs.name == gameName && gs.responder == responder.name)
				{
					//trace("Found gamename & responder's score " + gs.score);
					return gs;
				}
			}
			trace("Failed to find a score for " + gameName + " and " + responder.name);
			return null;
		}
		...
		public function getHighestGameScoresTo(charName:String, count:Number = 5, minVolition:Number = -9999):Vector.<GameScore> {
			...
			return scoresTo.sort(this.scoreCompare).slice(0, count);
		}
