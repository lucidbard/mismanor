package CiF
{
	...
	public class SocialNetwork
	{
		 ...
		 */
		public static const NETWORK_COUNT:Number = 4;
		 
		public static const BUDDY:Number 	= 0;
		public static const ROMANCE:Number	= 1;
		public static const TRUST:Number 	= 2;
		public static const FAMILYBOND:Number = 3;
		...
		* */
		...
		public static function getTypeFromName(name:String):Number {
			switch (name.toLowerCase()) {
				...
				case "trust":
					return SocialNetwork.TRUST;
				case "familybond":
					return SocialNetwork.FAMILYBOND;
				default:
					return -1;
			}
		}
		...
		/**
		 * getReverseRelationshipsAboveThreshold returns a vector of ints, representing the characterIDs
		 * of the characters whom have a relationship weight with charID higher than the threshold. This is good
		 * if you want a list of high relationships between an individual.
		 * @param	charID The character who is the recipient of the relationships
		 * @param	threshold The threshold that must be passed for the relationship to be added
		 */
		public function getReverseRelationshipsAboveThreshold(charID:int, threshold:Number):Vector.<Number> 
		{
			var idsAboveThreshold:Vector.<Number> = new Vector.<Number>();
			for (var i:Number = 0; i < network.length; i++)
			{
				if (i != charID) {
					if (network[i][charID] > threshold)
						idsAboveThreshold.push(i);
				}
			}
