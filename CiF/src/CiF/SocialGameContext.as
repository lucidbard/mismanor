	...
	public class SocialGameContext implements SFDBContext
	{
		...
		public function clean(): void {
			if (this.queryCKB)
				this.queryCKB.clean();
			this.queryCKB = null;
			this.chosenItemCKB = null;
			this.performanceRealizationString = null;
			this.gameName = null;
			this.initiator = null;
			this.label = null;
			this.labelArg1 = null;
			...
			if (this.backstoryLocutions)
			{
				...
				this.backstoryLocutions = null;
			}
			
			if (this.SFDBLabels)
			{
				for each (var sfdblab:SFDBLabel in this.SFDBLabels)
				{
					sfdblab.clean();
					sfdblab = null;
				}
				this.SFDBLabels = null;
			}
		}
		/**********************************************************************
		 ...
		 */
		public function getEffectPerformanceRealizationString(initiator:GameObject, responder:GameObject, other:GameObject,speaker:String):String {
			var cif:CiFSingleton = CiFSingleton.getInstance();
		...
		public function getInitiator():GameObject
		{
			for each (var c:Character in CiFSingleton.getInstance().cast.characters) {
				if(this.initiator == c.name) return c as GameObject;
			}
			for each (var i:Item in CiFSingleton.getInstance().itemList.items) {
				if(this.initiator == i.name) return i as GameObject;
			}
			for each (var k:Knowledge in CiFSingleton.getInstance().knowledgeList.knowledges) {
				if(this.initiator == k.name) return k as GameObject;
			}
			return null;
		}
		public function getResponder():GameObject
		{
			for each (var c:Character in CiFSingleton.getInstance().cast.characters) {
				if(this.responder == c.name) return c as GameObject;
			}
			for each (var i:Item in CiFSingleton.getInstance().itemList.items) {
				if(this.responder == i.name) return i as GameObject;
			}
			for each (var k:Knowledge in CiFSingleton.getInstance().knowledgeList.knowledges) {
				if(this.responder == k.name) return k as GameObject;
			}
			return null;
		}
		public function getOther():GameObject
		{
			for each (var c:Character in CiFSingleton.getInstance().cast.characters) {
				if(this.other == c.name) return c as GameObject;
			}
			for each (var i:Item in CiFSingleton.getInstance().itemList.items) {
				if(this.other == i.name) return i as GameObject;
			}
			for each (var k:Knowledge in CiFSingleton.getInstance().knowledgeList.knowledges) {
				if(this.other == k.name) return k as GameObject;
			}
			return null;
		}
		...
		public function isPredicateInChange(p:Predicate, x:GameObject, y:GameObject, z:GameObject):Boolean {
			...
			for each (var predInChange:Predicate in effect.change.predicates) {
				...
				if (Predicate.equalsValuationStructure(p, predInChange)) {
					...
					if (doPredicateRolesMatchCharacterVariables(predInChange, x, y, z)) 
		 ...
		 */
		public function isLabelAndCharactersInPredicate(p:Predicate, x:GameObject, y:GameObject, z:GameObject):Boolean {
		 ...
		 */
		private function doesPredicatePrimaryMatch(predInEvalRule:Predicate, predInChange:Predicate, x:GameObject, y:GameObject, z:GameObject):Boolean {
			...
			switch(predInEvalRule.getPrimaryValue()) {
				case "initiator":
				case "x":
					characterReferedToInEvalRule = x.name;
					break;
				case "responder":
				case "y":
					characterReferedToInEvalRule = y.name;
					break;
				case "other":
				case "z":
					characterReferedToInEvalRule = z.name;
					break;
				case "":
		 ...
		 * character variables, x,y, and z.
		 */
		private function doesPredicateSecondaryMatch(predInEvalRule:Predicate, predInChange:Predicate, x:GameObject, y:GameObject, z:GameObject):Boolean {
			var characterReferedToInEvalRule:String;
			var characterReferedToInPredInChange:String;
		 ...
		 * character variables, x,y, and z.
		 */
		private function doesPredicateTertiaryMatch(predInEvalRule:Predicate, predInChange:Predicate, x:GameObject, y:GameObject, z:GameObject):Boolean {
			var characterReferedToInEvalRule:String;
			var characterReferedToInPredInChange:String;
		 ...
		 * character variables x,y, and z respectively.
		 */
		private function doPredicateRolesMatchCharacterVariables(predInChange:Predicate, x:GameObject, y:GameObject, z:GameObject, predInEvalRule:Predicate=null):Boolean {
		 ...
		 */
		public function doesSFDBLabelMatch(label:int, firstCharacter:GameObject = null, secondCharacter:GameObject = null, thirdCharacter:GameObject = null, pred:Predicate = null):Boolean {
			var testBool:Boolean = false;
			if (this.isBackstory) {
				//Debug.debug(this, "Checking Backstory SFDB...");
				//-1 means any label matches
				if (label != -1){
					//if not wildcard, label must match
					if (this.label != SocialFactsDB.getLabelByNumber(label)) {
						//Debug.debug(this, "SFDB Label does not match: " + this.label + " != " + SocialFactsDB.getLabelByNumber(label));
						return false;
					}
				}
				...
				//in backstory contexts, the firstCharacter's name must always match the context's initiator if firstCharacter exists
					if (this.initiator.toLowerCase() != firstCharacter.name.toLowerCase()) {
						//Debug.debug(this, "SFDB INIT does not match: " + this.initiator.toLowerCase() + " != " + firstCharacter.name.toLowerCase());
						return false;
					}
				}
				...
				if (secondCharacter){
					...
					if (this.responder.toLowerCase() != secondCharacter.name.toLowerCase()) {
						//Debug.debug(this, "SFDB RESP does not match: " + this.responder.toLowerCase() + " != " + secondCharacter.name.toLowerCase());
						return false;
					}
				}
				// (April) WTF Why was this not in here? (Found it in Prom's code but not ours)
				//If we've made it to this point, it seems like we have a match!  we should return true!
				//Debug.debug(this, "TRUE!");
				return true;
			}else {
				if (this.SFDBLabels) {//the multi-SFDBLabel way
					for each(var sfdblabel:SFDBLabel in this.SFDBLabels) 
					{
						...
						if (firstCharacter)
						{
							if (firstCharacter.name != sfdblabel.from)
							{
								testBool = false;
							}
						}
						if (secondCharacter)
						{
							if (secondCharacter.name != sfdblabel.to)
							{
								testBool = false;
							}
						}
		 ...
		 * @return	True if this context is a match to the paramters. False if not a match.
		 */
		public function doesSFDBLabelMatchStrict(label:int, firstCharacter:GameObject, secondCharacter:GameObject = null, thirdCharacter:GameObject = null, pred:Predicate = null):Boolean {
			if (this.isBackstory) {
				//-1 means any label matches
				if (label != -1)
					//if not wildcard, label must match
				...
				if (firstCharacter)
				//in backstory contexts, the firstCharacter's name must always match the context's initiator if firstCharacter exists
					if (this.initiator.toLowerCase() != firstCharacter.name.toLowerCase()) return false;
				//if secondCharacter is null or if the label is not directed, treat it as a wildcard.
				//Note: right now we only have directed labels
				if (secondCharacter)
					//if not null, secondCharacter's name must match the context's responder
					if (this.responder.toLowerCase() != secondCharacter.name.toLowerCase()) return false;
			}else {
				if (this.SFDBLabels) {//the multi-SFDBLabel way
					for each(var sfdblabel:SFDBLabel in this.SFDBLabels) 
					{
						if (label == -1 || SocialFactsDB.doesMatchLabelOrCategory(SocialFactsDB.getLabelByName(sfdblabel.type),label))
						{
							...
							if (sfdblabel.from == firstCharacter.name) 
							{
								if (secondCharacter)
								{
									if (secondCharacter.name == sfdblabel.to)
